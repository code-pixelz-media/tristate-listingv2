...
//    alert('hello test');
  ...
  $("#tristate-input").val("");
    ...
    var brokerIds = $(this).val();
    var neighborhoodIds = $("#_gsheet_neighborhood").val();

    // Array to hold all AJAX promises
    var ajaxRequests = [];

    // Push each AJAX request promise into the array
    ...
    $.when.apply($, ajaxRequests).done(function() {
        // Arguments contain the responses for each AJAX request
        var responses = arguments;

        // Process each response
        $('#propertylisting-content').html(responses[0][0]);
        $('#_gsheet_use').html(responses[1][0]);
        $('#_buildout_city').html(responses[2][0]);
        $('#_gsheet_state').html(responses[3][0]);
      ...
      $('#_gsheet_listing_type,#_gsheet_neighborhood, #_gsheet_zip, #_gsheet_vented,#_gsheet_listing_type').on('input',function() {
