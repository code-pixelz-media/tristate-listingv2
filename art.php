...
add_shortcode('art', 'art_shortcode');

function art_shortcode($atts)
{
...
//    alert('hello test');
  ...
  $("#tristate-input").val("");
    ...
    if (JSON.stringify(prevSearchParams) === JSON.stringify(data) && prevSearchResult) {
      ...
      function performAjaxRequest(data) {
        ...
        $.each(allFields, function(i,e){
          ...
          $(this).on('input', function(){
            var fields = dropDownToUpdate($(this).prop('id'));
            var changedValues = {
              action: "tristate_update_dropdowns",
              agents: agentsField.val(),
              uses : usesField.val(),
              neighbourhoods:neighborhoodfield.val(),
              zips : gsheetZip.val(),
              cities:cities.val(),
              states:states.val(),
              venteds:vented.val(),
            ...
            $.post('<?php echo admin_url('admin-ajax.php'); ?>', changedValues, function (rdata) {
      ...
      // City change
      $('#_buildout_city').on('input',function() {
      ...
      // state change
      $('#_gsheet_state').on('input',function() {
      ...
      $('#_gsheet_listing_type,#_gsheet_neighborhood, #_gsheet_zip, #_gsheet_vented').on('input',function() {
...
?>
<?php
// tristate_update_dropdowns
add_action('wp_ajax_tristate_update_dropdowns', 'tristate_update_dropdowns');
add_action('wp_ajax_nopriv_tristate_update_dropdowns', 'tristate_update_dropdowns');

function tristate_update_dropdowns(){
